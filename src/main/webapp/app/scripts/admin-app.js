...
(function() {
  ...
  angular.module('stockApp', [ 'ngAnimate', 'ngCookies', 'ngSanitize', 'ui.router', 'ngMaterial', 'nvd3', 'md.data.table', 'mdDatetime' ])
  ...
  .run(function($rootScope, $mdToast, $mdDialog, $state, $timeout, RestService, contextRoot, restUrl) {
    ...
    $rootScope.showMsg = function(title, delay, className) {
      $mdToast.show($mdToast.simple().toastClass(className).content(title).hideDelay(delay ? delay : 1000).position('bottom right'));
    };
