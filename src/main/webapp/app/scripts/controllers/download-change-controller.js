...
(function() {
	...
	angular.module('stockApp').controller('DownloadAccountChangeController',
			...
			$scope.tableCols = [ 
       {
			...
			$scope.RestService.post({
				urlPath : $scope.restUrl.queryHeadChangeType,
			}, function(response) {
			...
			$scope.retry = function() {
				$scope.confirmDialog({
					title : '确认下载账户变动记录',
					content : $scope.selected.length > 0 ? '下载选定的账户变动记录？' : '根据查询条件下载账户变动记录？',
					confirm : function(isConfirm) {
						if (isConfirm) {
							$scope.promise = $scope.RestService.post({
								urlPath : $scope.restUrl.retryHeadAccountChange,
								data : {
									selected : $scope.selected,
									query : $scope.query
								}
							}, function(response) {
								$scope.showMsg('下载账户变动记录成功');
			...
			$scope.exportExcel = function() {
				$scope.promise = $scope.RestService.post({
					urlPath : $scope.restUrl.exportHeadAccountChange,
					responseType : 'blob',
					data : {
						query : $scope.query,
					...
					$scope.windowExportExcel(response.data,'总店会员账户变动信息');
				});
			};
			...
			$scope.queryItems = function(reset) {
				...
				$scope.promise = $scope.RestService.post({
					urlPath : $scope.restUrl.queryHeadAccountChange,
					data : $scope.query
				}, function(response) {
