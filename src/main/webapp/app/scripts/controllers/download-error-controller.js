...
(function() {
	...
	angular.module('stockApp').controller('DownloadErrorController',
		[ '$scope',function($scope) {
			...
			$scope.tableCols = [ {
				...
				id : 'errordatetime',
				name : '异常时间',
				orderBy : 'errordatetime'
			} ];

			$scope.dataTypes = [];

			$scope.RestService.post({
				urlPath : $scope.restUrl.queryHeadDataType
			}, function(response) {
			...
			$scope.retry = function() {
				$scope.confirmDialog({
					title : '确认重载异常信息',
					content : $scope.selected.length > 0 ? '重新下载选定的异常信息？' : '根据查询条件重新下载异常信息？',
					confirm : function(isConfirm) {
						if (isConfirm) {
							$scope.promise = $scope.RestService.post({
								urlPath : $scope.restUrl.retryHeadDownloadError,
								data : {
									selected : $scope.selected,
									query : $scope.query
								}
							}, function(response) {
								$scope.showMsg('重新下载异常信息成功');
			...
			$scope.queryItems = function(reset) {
				...
				$scope.promise = $scope.RestService.post({
					urlPath : $scope.restUrl.queryHeadDownloadError,
					data : $scope.query
				}, function(response) {
